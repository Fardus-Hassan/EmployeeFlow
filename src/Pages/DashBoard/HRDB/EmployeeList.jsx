...
const EmployeeList = () => {
    ...
    const handleVerify = async (id, name) => {
        console.log(id);
        try {
            const { data } = await axios.patch(`http://localhost:3000/users/${id}`, { verify: true });
            if (data.acknowledged) {
                toast.success(`Verified ${name}`);
                setEntries(entries.map(entry => entry._id === id ? { ...entry, verify: true } : entry));
            }
        }
        catch (error) {
    ...
    const closeVerifyModal = () => setVerify(false);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const filteredData = data.filter(item => item.role === 'Employee')
                        ...
                        <tbody className="bg-white dark:bg-themeColor">
                            {entries.map((entry, index) => (
                                <tr key={entry._id} className={index % 2 === 0 ? 'bg-gray-100 dark:bg-themeColor' : 'bg-white dark:bg-themeColor2'}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-black text-sm dark:text-white font-semibold text-center">
                                        <button disabled={!entry.verify} onClick={() => openModal(entry)} className={`rounded-xl ${entry.verify ? " bg-secColor text-white" : "cursor-not-allowed bg-gray-500 text-gray-300"} px-3 py-1 font-poppins font-bold`}>Pay</button>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-black text-sm dark:text-white font-semibold text-center">
                        ...
                        <div className='max-w-full'>
                            <div>
                                ...
                                <div className="mt-2 text-center">
                                    ...
                                    <p className="mt-2 text-sm text-wrap mx-auto text-gray-500 dark:text-gray-400">
                                        If you want to go here! Please login
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div className="mt-5 sm:flex sm:items-center sm:justify-center sm:max-w-full">
                            <div className="sm:flex sm:items-center sm:max-w-full">
                                <button onClick={closeModal} className="w-full px-4 py-2 mt-2 text-sm block font-medium tracking-wide text-gray-700 capitalize transition-colors duration-300 transform border border-gray-200 rounded-md sm:mt-0 sm:w-auto sm:mx-2 dark:text-gray-200 dark:border-gray-700 dark:hover:bg-gray-800 hover:bg-gray-100 focus:outline-none focus:ring focus:ring-gray-300 focus:ring-opacity-40">
                                    Cancel
                                </button>
